...
module.exports = {
    ...
    addCollectionType: async (req, res, next) => {
        ...
        const createPermission = accessControl.can(user.userType).createAny(resources.collectionType);
    ...
    deleteCollectionType: async (req, res, next) => {
        ...
        const deleteAnyPermission = accessControl.can(user.userType).deleteAny(resources.collectionType);
        const deleteOwnPermission = accessControl.can(user.userType).deleteOw(resources.collectionType);
    ...
    getCollectionType: async (req, res, next) => {
        ...
        if (readPermission.granted) {
            ...
            if (readAnyInActiveResource.granted){
                ...
                requestedCollectionType = await CollectionType.findOne({_id:requestedCollectionTypeId,$or:[{createdBy:daiictId},{isActive:true}]});
            } else {
                requestedCollectionType = await CollectionType.findOne({_id:requestedCollectionTypeId,isActive:true});
            }
    ...
    updateCollectionType: async (req, res, next) => {
        ...
        const updatePermission = accessControl.can(user.userType).updateAny(resources.collectionType);
        const readPermission = accessControl.can(user.userType).readAny(resources.collectionType);
