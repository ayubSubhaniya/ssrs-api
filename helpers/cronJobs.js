...
const checkForOfflinePayment = async () => {
    ...
    for (let i = 0; i < carts.length; i++) {
        if (carts[i].statusChangeTime.placed.time <= failedOrderTime) {
            ...
            const cancelledInDays = (carts[i].statusChangeTime.placed.time.getTime() - new Date().getTime())/MILLI_SECOND_IN_ONE_DAY + ORDER_CANCEL_TIME_IN_PAYMENT_DELAY;
...
const checkForFailedOnlinePayment = async () => {
    ...
    for (let i = 0; i < carts.length; i++) {
        if (carts[i].statusChangeTime.paymentFailed.time <= failedOrderTime) {
            ...
            const cancelledInDays = (carts[i].statusChangeTime.paymentFailed.time.getTime() - new Date().getTime())/MILLI_SECOND_IN_ONE_DAY + ORDER_CANCEL_TIME_IN_PAYMENT_DELAY;
